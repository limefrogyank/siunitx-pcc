...
import { Configuration, ParserConfiguration } from "@mathjax/src/js/input/tex/Configuration.js";
import { CommandMap, CharacterMap } from "@mathjax/src/js/input/tex/TokenMap.js";
...
function angleChars(parser: TexParser, mchar: string | TexToken | StackItem) {
...
new CommandMap(
    ...
    {
        siunitxToken: (parser: TexParser, name: string | TexToken | StackItem) => {
            if (typeof name === "string") methodMap[name as string]?.(parser);
        },
        Arialabel: (parser: TexParser, name: string | TexToken | StackItem) => {
